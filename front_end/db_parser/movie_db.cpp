...
namespace persudo {
    namespace db_parser {
        ...
        vector<string> MovieDB::search_movie(const string &keyword) {
            return vector<string>{keyword};
            //std::cout << "call search movie" << std::endl;
            //mongocxx::collection coll = (this->db)[movie_invert_list_name];
            //vector<string> ret;
            //// fetch and build the MovieData
            //mongocxx::stdx::optional<bsoncxx::document::value> result =
            //        coll.find_one(document{} << "keyword" << keyword << finalize);
            //if (result) {
            //    Json::Value docs;
            //    std::stringstream ss;
            //    ss << bsoncxx::to_json(*result);
            ...
            //    for (int i = 0; i < list.size(); ++i) {
            //        ret.push_back(list[i].asString());
            //        std::cout << "search out: " << list[i].asString() << std::endl;
            //    }
            //}
            //return ret;
        }
